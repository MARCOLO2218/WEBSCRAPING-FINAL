// Catálogo regional en la base DEV; sin --apply sólo comprueba requisitos.
...
if (args.some(arg => arg !== '--apply') || args.length > 1) {
  console.error('Uso: node scripts/countries-dev.mjs [--apply]');
  process.exit(2);
}
dotenv.config({ path: resolve(root, '.env'), quiet: true });
const python = process.platform === 'win32' ? '.venv/Scripts/python.exe' : '.venv/bin/python';
const result = spawnSync(resolve(root, 'backend', python),
  ['-m', 'catalog_api.db.countries', ...args],
