...
const columns = ['codigo_producto', 'producto', 'marca', 'linea', 'categoria', 'precio_regular',
  'precio_oferta', 'moneda', 'disponibilidad', 'activo', 'fecha_vigencia', 'observaciones'];
...
export async function validatePrices(buffer: Buffer) {
  ...
  const preview: Array<{ producto: string; precio_regular: string; precio_oferta: string }> = [];
  let count = 0;
  sheet.eachRow((row, number) => {
    ...
    const placeholders: Record<string, string> = { marca: 'FACENCO', moneda: 'GTQ', disponibilidad: 'DISPONIBLE', activo: 'SI' };
    ...
    count++;
    if (preview.length < 5) preview.push({ producto: product, precio_regular: text('precio_regular'), precio_oferta: text('precio_oferta') });
  });
  if (!count) errors.push('No hay productos activos para cargar.');
  if (errors.length) throw new UploadError(errors.slice(0, 50).join('\n'));
  return { count, preview, buffer: Buffer.from(await book.xlsx.writeBuffer()) };
}
...
export function createPriceUpload(target = resolve('data/precios_facenco.xlsx')) {
  ...
  return async (buffer: Buffer, confirm: boolean) => {
    ...
    try {
      ...
      if (!confirm) return { ok: true, saved: false, count: checked.count, preview: checked.preview };
      ...
      return { ok: true, saved: true, count: checked.count, backup: backedUp ? backup : null };
    } finally {
